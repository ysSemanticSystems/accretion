...
The gate is `make check` (generators, C1–C7, W1–W9, `cargo test`, clippy, fmt,
`make godot-presentation`) green.

## Coverage model (aligned with CI)

A public Rust function is **covered** when any of the following holds:

1. A **unit or integration test** with a hand-computed or analytic expected value.
2. A **golden oracle case** in `golden.json` (generated by `scripts/gen_golden.py`).
3. A **property test** (monotonicity, round-trip, scaling) with stated tolerance.

Golden-backed symbols today: `SIGMA_SB`, `SIGMA_T`, `l_eddington`,
`disk_temperature`, `outer_horizon_radius_rg`, `isco_radius`,
`efficiency_from_spin`, `blackbody_rgb`.

Presentation/shell behavior: extend `presentation_tests.gd` — no physics formulas
in those tests.
